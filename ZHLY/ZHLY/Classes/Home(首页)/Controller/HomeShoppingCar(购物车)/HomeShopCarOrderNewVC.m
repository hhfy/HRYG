...
#import "HomeShopCarOrderNewVC.h"
...
@interface HomeShopCarOrderNewVC () <UITableViewDelegate, UITableViewDataSource>
...
@implementation HomeShopCarOrderNewVC
...
- (void)viewDidLoad {
    ...
    [LaiMethod setupDownRefreshWithTableView:self.tableView target:self action:@selector(getOrderData)];
...
- (void)getOrderData {
    ...
    [HttpTool postWithURL:url params:params isHiddeHUD:YES success:^(id json) {
        ...
        self.commitView.price = [weakSelf.ticketMainOrder.need_pay floatValue];
...
- (void)postOrderData {
    ...
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithDictionary:self.paramsArr[0]];
    params[@"pay_type"] = @(self.payType);//[LaiMethod getPayType:self.payType];
...
//        }
        ...
        //支付宝支付设置
//        AlipayInfo *info = [AlipayInfo mj_objectWithKeyValues:json[Data]];
...
//        if (self.ticketMainOrder.ticket_link.count) {
//           return [self homeShopBaseOrderSpecialTicketCellWithTableView:tableView indexPath:indexPath];
//        }
//        else {
//            if (ticket.start_date && ticket.end_date) {
//                return [self homeHotelOrderAddTouristCellWithTableView:tableView];
//            }
//            else {
...
- (HomeHotelOrderAddTouristCell *)homeHotelOrderAddTouristCellWithTableView:(UITableView *)tableView {
