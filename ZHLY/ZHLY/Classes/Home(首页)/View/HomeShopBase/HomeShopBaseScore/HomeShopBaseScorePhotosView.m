...
@implementation HomeShopBaseScorePhotosView
...
- (void)setPhotos:(NSArray *)photos {
    ...
    while (self.subviews.count < photoCount) {
        HomeMuseumScorePhotoView *photoView = [HomeMuseumScorePhotoView new];
        [self addSubview:photoView];
    }
    
    for (int i = 0; i < self.subviews.count; i++) {
        HomeMuseumScorePhotoView *photoView = self.subviews[i];
        photoView.tag = i;
        if (i < photoCount) {
...
@implementation HomeMuseumScorePhotoView
