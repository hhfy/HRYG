...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    ...
    //屏蔽掉全非状态时的操作
    if (self.refreshStatus == RefreshNone) {
        return;
    }
    ...
    if (self.refreshStatus == 0) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            self.refreshStatus = RefreshDefault;
        });
        return;
    }
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            /**非刷新状态**/
            if (self.refreshStatus == RefreshDefault) {
                [self defaultHandleWithOffSet:offsetY change:change];
                
                /**刷新状态**/
            } else if (self.refreshStatus == RefreshBeginRefresh) {
...
- (void)beginRefresh {
    //状态取反 保证一次刷新只执行一次回调
    if (self.refreshStatus != RefreshBeginRefresh) {
        self.refreshStatus = RefreshBeginRefresh;
...
- (void)anmiationHandelwithChange:(NSDictionary<NSKeyValueChangeKey,id> *)change andStatus:(StatusOfRefresh)status needAnimation:(BOOL)need {
    ...
    if (status == RefreshDefault) {
        ...
        /**
         刷新状态下的动作处理
         */
    } else if (status == RefreshBeginRefresh) {
        if (!self.animation) {
...
- (void)endHeaderRefresh {
    ...
    if (self.refreshStatus != RefreshNone) {
        self.refreshStatus = RefreshNone;
...
- (void)creatMainUI {
    ...
    _refreshIcon.image = [[UIImage imageWithContentsOfFile:[[NSBundle bundleWithPath:[[NSBundle bundleForClass:[RefreshView class]] pathForResource:@"RefreshResource" ofType:@"bundle"]] pathForResource:@"refresh" ofType:@"png"]] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
