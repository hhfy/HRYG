...
'use strict';
  ...
  fn.init = function () {
    ...
    this.optional = this.opts.optional
    ...
    for (var i = 0; i < rows; i++){
      ...
      for (var k = 1; k <= 7; k++){
        ...
        if (d > 0 && d <= thisMonthDays){
          ...
          // 为可操作，且显示价格、库存
          if (tdId >= startDay && tdId <= endDay && $.inArray(tdId, this.optional) >= 0) {
            if(tdId === click_date) {
                html += '<td class="valid-hook active" data-week="' + (k - 1) + '" data-id="' + tdId + '"><b>' + d + '</b><div class="data-hook"></div></td>';
            }else {
  ...
  fn.initSettingWindow = function () {
    if($('.capricorncd-date-detailed-settings').length > 0) return false
    var html = '';
  ...
  fn._createDaySetupInputGroup = function () {
    ...
    for (var i = 0; i < config.length; i++) {
      ...
      html += '   <input name="'+ val.key +'" type="text" onkeyup="checkPrice(this)">';
      html += '</li>';
    }
    return html;
  };
  ...
  fn.handleClickEvent = function () {
    ...
    this.calendar.on('click', '.valid-hook', function () {
      // 当天日期
      var thisDate = click_date = $(this).data('id');
      // 当前日的数据
      ...
      // 拦截弹出设置窗口，返回当天数据
      if (me.opts.everyday) {
        me.opts.everyday(data, thisDate);
        return;
      }
